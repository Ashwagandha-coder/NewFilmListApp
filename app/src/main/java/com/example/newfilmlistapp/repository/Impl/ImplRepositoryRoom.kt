...
class ImplRepositoryRoom(room: AppDatabase) : RepositoryRoom {

    private val movieDao = room.movieDao()
    ...
    override suspend fun getMovieListLocal(): List<MovieDetailWrapperRoom>? =
        withContext(Dispatchers.IO) {
            movieDao.getMovieList()
        }

    override suspend fun getMovieLocal(id: String): MovieDetailWrapperRoom? =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun insertLocal(list: List<MovieDetailWrapperRoom>) =
        withContext(Dispatchers.IO) {
    ...
    override suspend fun deleteMovieLocal(movie: MovieDetailWrapperRoom) =
        withContext(Dispatchers.IO) {
